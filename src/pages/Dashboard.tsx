...
  const [counts, setCounts] = useState({ alunos: 0, instrutores: 0, treinos: 0, exercicios: 0 })

  useEffect(()=> {
    async function load() {
      try {
        const [al, ins, tr, ex] = await Promise.all([
          api.get('/alunos'),
          api.get('/instrutores'),
          api.get('/treinos'),
          api.get('/exercicios')
        ])
        setCounts({ alunos: al.data.length || 0, instrutores: ins.data.length || 0, treinos: tr.data.length || 0, exercicios: ex.data.length || 0 })
      } catch (err) {
        // ignore for now
      }
    }
...
        </Link>
        <Link to="/instrutores" className="p-6 bg-green-50 text-green-800 rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300">
          <div className="text-sm font-semibold">Instrutores</div>
          <div className="text-3xl font-extrabold mt-1">{counts.instrutores}</div>
        </Link>
